...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    // malloc the new WordPosting sturcture
    wp = (WordPostings *) malloc(sizeof(WordPostings));
    Verify333(wp != NULL);

    // allocate a new hashtable for the wordposting posting pointer
    wp->postings = HashTable_Allocate(8);
    wp->word = word;

    mi_kv.key = key;
    mi_kv.value = (HTValue_t) wp;
    if (HashTable_Insert(index, mi_kv, &unused)) {
      printf("Key 1 Already Exists, this shouldn't happen\n");
      return;
    }
  } else {
  ...
  if (HashTable_Insert(wp->postings, postings_kv, &unused)) {
    printf("Sub Key Exists, this shouldn't happen\n");
    return;
  }
  return;
}
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  LinkedList* postingPtr;
  key = FNVHash64((unsigned char*) query[0], strlen(query[0]));
  if (HashTable_Find(index, key, &kv)) {  // if we find the word
    wp = kv.value;
    // int n = HashTable_NumElements(wp->postings);
    SearchResult* srPtr = (SearchResult *) malloc(sizeof(SearchResult));
    HTIterator* iter = HTIterator_Allocate(wp->postings);
    HTIterator_Get(iter, &kv);  // get the rows in hashtable 2
    postingPtr = (LinkedList *) kv.value;
    int Rank = LinkedList_NumElements(postingPtr);
    srPtr->doc_id = (DocID_t) kv.key;
    srPtr->rank = Rank;
    ret_list = LinkedList_Allocate();  // allocate linkedlist first
    LinkedList_Append(ret_list, (LLPayload_t *) srPtr);  // then append
    while (HTIterator_Next(iter)) {
      HTIterator_Get(iter, &kv);
      postingPtr = (LinkedList *) kv.value;
      Rank = LinkedList_NumElements(postingPtr);
      // allocate new heap memory for newsrptr
      SearchResult* newSrPtr = (SearchResult *) malloc(sizeof(SearchResult));

      newSrPtr->doc_id = (DocID_t) kv.key;
      newSrPtr->rank = Rank;
      LinkedList_Append(ret_list, (LLPayload_t *) newSrPtr);
    }
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      // declare a new srptr
      // SearchResult * srptr = (SearchResult *) malloc(sizeof(SearchResult));
      // maybe don't allocate space
      SearchResult * srptr;
      LLIterator_Get(ll_it, (LLPayload_t *) &srptr);
      if (HashTable_Find(wp->postings, (HTKey_t)srptr->doc_id, &kv)) {
        // add the ranking and update the payload
        // by updating here means to remove the iter one and append a new one
        // postingPtr now points to the position linkedlist
        postingPtr = (LinkedList *) kv.value;
        // set a new SearchResult pointer to append it
        SearchResult * srFinal = (SearchResult *) malloc(sizeof(SearchResult));
        srFinal->rank = srptr->rank + LinkedList_NumElements(postingPtr);
        srFinal->doc_id = srptr->doc_id;
        LLIterator_Remove(ll_it, (LLPayloadFreeFnPtr)free);
        LinkedList_Append(ret_list, (LLPayload_t *) srFinal);
      } else {
        // delete using LLIterator, and don't call next
        LLIterator_Remove(ll_it, (LLPayloadFreeFnPtr)free);
      }
    }
    HTIterator_Free(iter);
