...
static void ValueFree(HTValue_t value) {free(value);}
...
void DocTable_Free(DocTable* table) {
  ...
  // STEP 1.
  HashTable_Free(table->id_to_name, ValueFree);
  HashTable_Free(table->name_to_id, ValueFree);
  free(table);
}
...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  if (HashTable_Find(DT_GetNameToIDTable(table), res, &old_kv)) {
    return *(DocID_t *)old_kv.value;
  }

  doc_copy = (char *) malloc(strlen(doc_name) + 1);
  snprintf(doc_copy, strlen(doc_name)+1, doc_name);
