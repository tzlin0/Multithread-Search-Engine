...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (stat(file_name, &file_stat)!= 0) {
    perror("filename not read correctly by stat");
    return NULL;
  }
  // STEP 2.
  ...
  buf = (char*) malloc((file_stat.st_size + 1) * sizeof(char));
  ...
  while (left_to_read > 0) {
    ...
    if (num_read == -1) {
      ...
      // EINTR happens, try again
      continue;
    } else if (num_read == 0) {
      // stop reading, reach EOF
      break;
    }
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  // Until we reach the EOF, process each char
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr)) {
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    char *copystr = (char *) malloc(strlen(word) + 1);
    wp->word = copystr;
    snprintf(copystr, strlen(word)+1, word);
