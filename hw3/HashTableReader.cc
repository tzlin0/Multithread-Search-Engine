...
HashTableReader::HashTableReader(FILE* f, IndexFileOffset_t offset)
  ...
  BucketListHeader header;
  Verify333(fseek(file_, offset_, SEEK_SET) == 0);
  Verify333(fread(&header, sizeof(BucketListHeader), 1, file_) == 1);
  header.ToHostFormat();
  header_ = header;
}
...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  IndexFileOffset_t read_pos = bucket_rec.position;
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord position;
    Verify333(fseek(file_, read_pos, SEEK_SET) == 0);
    Verify333(fread(&position, sizeof(ElementPositionRecord), 1, file_) == 1);
    position.ToHostFormat();
    ret_val.push_back(position.position);
    read_pos += sizeof(ElementPositionRecord);
  }
