...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  int hdr_bytes = WriteHeader(f, dt_bytes, mi_bytes);
  if (hdr_bytes == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  CRC32* crc_ptr = new CRC32();
  CRC32 crc = *crc_ptr;
  if (fseek(f, sizeof(IndexFileHeader), SEEK_SET) != 0) {
    return kFailedWrite;
  }
  char buf[1];
  for (size_t res = fread(buf, 1, 1, f); res > 0; res = fread(buf, 1, 1, f)) {
         crc.FoldByteIntoCRC((uint8_t) buf[0]);
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    int32_t chain_len = LinkedList_NumElements(ht->buckets[i]);

    int br_bytes = WriteHTBucketRecord(f, record_pos, chain_len, bucket_pos);
    if (br_bytes == kFailedWrite) {
      return kFailedWrite;
    }
    record_pos += br_bytes;

    if (chain_len > 0) {
      int buc_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      if (buc_bytes == kFailedWrite) {
        return kFailedWrite;
      }
      bucket_pos += buc_bytes;
    }
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord position(element_pos);
    position.ToDiskFormat();
    if (fwrite(&position, sizeof(ElementPositionRecord), 1, f) != 1) {
      return kFailedWrite;
    }

    // STEP 8.
    // Write the element itself, using fn.
    LLIterator_Get(it, reinterpret_cast<LLPayload_t*>(&keyval));
    int element_size = fn(f, element_pos, keyval);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  char* name = static_cast<char*> (kv->value);

  int16_t file_name_bytes = strlen(name);  // you may want to change this
  ...
  if (fwrite(name, file_name_bytes, 1, f) != 1) {
    return kFailedWrite;
  }
  ...
  return (sizeof(DocIDElementHeader) + file_name_bytes);
}
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    LLIterator_Get(it, reinterpret_cast<LLPayload_t*>(&value));


    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    position.position = static_cast<DocPositionOffset_t>(value);
    position.ToDiskFormat();

    if (fseek(f, offset + sizeof(DocIDElementHeader) +
             (sizeof(DocIDElementPosition) * i), SEEK_SET) != 0) {
      return kFailedWrite;
    }
