...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  int unit = sizeof(IndexFileHeader);
  Verify333(fread(&header, unit, 1, file_) == 1);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      int oneread = fread(buf, 1, kBufSize, file_);
      left_to_read -= oneread;
      for (int i = 0; i < oneread; i++) {
        crc_obj.FoldByteIntoCRC((uint8_t) buf[i]);
      }
    }
