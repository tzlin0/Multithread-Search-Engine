...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char *rd = const_cast<char*>(root_dir.c_str());
  char *tf = const_cast<char*>(test_file.c_str());
  char *rd_abs;
  char *tf_abs;
  tf_abs = realpath(tf, NULL);
  rd_abs = realpath(rd, NULL);
  if (tf_abs == NULL || rd_abs == NULL) {
    return false;
  }
  char song = '/';
  strncat(tf_abs, &song, 1);
  strncat(rd_abs, &song, 1);
  if (strstr(tf_abs, rd_abs) != NULL) {
    return true;
  }
  return false;
}
