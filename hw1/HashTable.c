...
static void LLNoOpFree(LLPayload_t freeme) {free(freeme);}
...
bool HashTable_Insert(HashTable *table,
  ...
  newpayload = (HTKeyValue_t *) malloc(sizeof(HTKeyValue_t));
  ...
  result  = Bucket_Lookup(chain, newkeyvalue.key, oldkeyvalue, true);

  LinkedList_Append(chain, (LLPayload_t*) newpayload);
...
bool HashTable_Find(HashTable *table,
  ...
  return Bucket_Lookup(chain, key, keyvalue, false);
}
...
bool HashTable_Remove(HashTable *table,
  ...
  result = Bucket_Lookup(chain, key, keyvalue, true);
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  else if (iter->ht->buckets == NULL) return false;
  else if (iter->bucket_it == NULL) return false;

  return iter->ht->num_buckets > iter->bucket_idx;
}
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // if LLIterator is not at its last
  if (LLIterator_Next(iter->bucket_it)) return true;

  // else, node has been assigned to NULL
  ...
  while (iter->bucket_idx < iter->ht->num_buckets) {
    ...
    LLIterator_Free(iter->bucket_it);
    iter->bucket_it = LLIterator_Allocate(
                      (iter->ht->buckets[iter->bucket_idx]));

    // check if iterator is pointing to a valid node
    if (LLIterator_IsValid(iter->bucket_it)) {
...
bool Bucket_Lookup(LinkedList *bucket,
                    HTKey_t key,
                    HTKeyValue_t *keyvalue,
                    bool remove) {
  LLIterator *iter;
  ...
  Verify333(bucket != NULL);

  iter  = LLIterator_Allocate(bucket);

  // check linked list for key
  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t*) &curr);
